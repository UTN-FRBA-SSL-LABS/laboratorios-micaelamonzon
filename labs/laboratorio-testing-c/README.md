...
> R: 'carrito_init' inicializa el carrito estableciendo su cantidad en 0. Es importante llamarla antes de usar el carrito porque indica que inicialmente no hay productos cargados.
...
> R:Falla, esperaba 700 porque la cantidad es 350*2 y obtuvo 350.
...
> R:El bug está en la línea 24 y en el código dice que al total se lo calcula sumándole el precio del item en la posición i. 

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Esa línea debería multiplicar el precio unitario del producto por su cantidad y sumar ese resultado al total. 
...
> R:Falla, devuelve 1 cuando debería devolver 0.
...
> R:El operador incorrecto es <=, debería ser unicamente '<'.
...
> R: Sí, las líneas 29 y 30 de carrito.c, correspondientes a la función carrito_descuento, aparecen con #####. Esto es debido a que los tests unitarios ejecutados no llaman a carrito_descuento(), por lo que esas líneas nunca se ejecutaron durante la cobertura.
...
> R:La diferencia es que los tests unitarios son más específicos, verifican una función o variable de manera aislada, mientras que los de integración son más generales, verifican un número mayor de funciones. El primero en detectar el bug de carrito_total fue el test unitario.
...
> R:Es peligroso ya que se está queriendo escribir en una posición fuera del array, lo cual puede generar distintos errores, como un fallo del programa, ya que en el lenguaje C los arrays tienen tamaño fijo y no se reliza una verificación de límites. Por otro lado, Java y Python realizan controles de límites en los arrays y generan una excepción en lugar de permitir que se escriba fuera del rango. 
...
> R:Este enfoque es mejor ya que ayuda a señalar de manera más eficiente y confiable los errores; te ahorra tiempo y además se los encuentra con mayor facilidad.
...
> R:El primer test no detectó el bug ya que el producto tenía cantidad 1, por lo que el precio total no cambiaba si se multiplicaba por la cantidad o no, ya que al multiplicar por 1 el resultado iba a ser el mismo. 
